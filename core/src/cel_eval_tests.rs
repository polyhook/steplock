...
fn expr_true_for_matching_command() {
    ...
    assert!(matches_event(&ev, &Some("input.command.contains('git push')".to_owned())).unwrap());
}
...
fn pre_commit_expr_does_not_trigger_on_gh_pr_create_with_commit_in_body() {
    ...
    let safe_expr =
        "input.command_words.exists(x, x == 'commit') && input.command_words.exists(x, x == 'git')";
    assert!(!matches_event(&ev, &Some(safe_expr.to_owned())).unwrap());
}
